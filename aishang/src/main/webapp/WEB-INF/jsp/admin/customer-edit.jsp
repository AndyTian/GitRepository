...
<div style="padding:10px 10px 10px 10px">
	<form id="customerEditForm" class="itemForm" method="post">
	...
	<div style="padding:5px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="customerEditPage.submitForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="customerEditPage.clearForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
var customerEditEditor ;
$(function(){
	customerEditEditor = AS.createEditor("#customerEditForm [name=message]");
});

var customerEditPage = {
		submitForm : function(){
			if(!$('#customerEditForm').form('validate')){
				$.messager.alert('提示','表单还未填写完成!');
				return ;
			}
			customerEditEditor.sync();
			
			$.post("/manage/customer/edit",$("#customerEditForm").serialize(), function(data){
				if(data.status == 200){
