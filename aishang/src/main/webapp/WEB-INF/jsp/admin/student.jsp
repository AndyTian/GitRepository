...
<div class="easyui-panel" title="Nested Panel" data-options="width:'100%',minHeight:800,noheader:true,border:false" style="padding:10px;">
    <div class="easyui-layout" data-options="fit:true">
        <div data-options="region:'center'" style="padding:5px">
            <table class="easyui-datagrid" id="studentList" data-options="toolbar:studentListToolbar,singleSelect:false,collapsible:true,pagination:true,method:'get',pageSize:20,url:'/manage/student/list'">
		    <thead>
		        <tr>
...
var studentListToolbar = [{
    ...
    iconCls:'icon-edit',
    handler:function(){
    	var ids = AS.getSelectionsIds("#studentList");
    	if(ids.length == 0){
		...
		AS.createWindow({
			url : "/manage/student-edit",
			onLoad : function(){
				var data = $("#studentList").datagrid("getSelections")[0];
				$("#studentEditForm").form("load",data);
				
				// 实现图片
				if(data.pic1){
					$("#studentEditForm [name=pic1]").after("<a href='"+data.pic+"' target='_blank'><img src='"+data.pic1+"' width='80' height='50'/></a>");	
				}
				if(data.pic2){
					$("#studentEditForm [name=pic2]").after("<a href='"+data.pic2+"' target='_blank'><img src='"+data.pic2+"' width='80' height='50'/></a>");					
				}
				if(data.pic3){
					$("#studentEditForm [name=pic3]").after("<a href='"+data.pic2+"' target='_blank'><img src='"+data.pic3+"' width='80' height='50'/></a>");					
				}
				studentEditEditor.html(data.summary);
			}
		});    	
    }
},{
    ...
    handler:function(){
    	...
    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的内容吗？',function(r){
    	    if (r){
    	    	var params = {"ids":ids};
            	$.post("/manage/student/delete",params, function(data){
        			if(data.status == 200){
